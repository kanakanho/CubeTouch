...
1. 同じ現実空間の部屋の中に複数人のプレイヤーがいます
1. プレイヤーはそれぞれ対応した色を与えられます
1. プレイヤーは自分の対応した色のキューブをタッチすることでポイントが加算されます
1. キューブにタッチされると、キューブは消滅し、別の場所に出現します
1. キューブは 1cm 角のサイズで出現し、時間経過に合わせて 1m 角まで大きくなります（1m 角まで大きくなってしばらくすると消滅し、別の場所に出現します）
1. 時間内で他のプレイヤーより多くのキューブに触れれば勝利

## 面白いポイント

### どの部屋をゲーム場所にするのかによってプレイヤーの有利不利が変わる

例えば

- 物の多い部屋では小さいキューブは見つけにくい
- 白色の部屋では、白色のキューブは見つけずらい

よって、ゲーム開始時の色選択、ゲーム場所の選択が重要になる

### 他の人を妨害するのが重要

自分のキューブを見つけて触るだけではなく、他のプレイヤーのキューブを早く見つけて、相手に見つからないように体や物を使って隠すのが大事

ただ妨害ばかりしていても自分のポイントは増えないし、キューブは別の場所にリポップするため塩梅が大切になる

# キューブタッチゲーム 実装方針

## 概要

visionOS 上で複数プレイヤーが同一物理空間でプレイするARゲーム。
ImmersiveRPCKit（MultipeerConnectivity ベース）を使ってデバイス間通信を行う。

RPCの実装については適宜 https://github.com/kanakanho/ImmersiveRPCKit を参照すること
...
全員対等なP2Pトポロジー。特定のホストデバイスは存在しない。

- ゲーム開始：全員が「始める」を押したら `startGame` を broadcast
- ゲーム終了：`spawnTime` から各デバイスがローカルでカウントダウンし、時間になったら自律的に終了
- キューブ管理：タッチしたプレイヤーが次のキューブをスポーンする権限を持つ

### 座標系

`SharedCoordinateImmersiveView` + `CoordinateTransforms` を使って全デバイスのワールド座標系を統一する。
キューブの位置は **タッチしたデバイスのワールド座標系で生成**し、`run(transforming:)` で各デバイスの座標系に変換して送信する。
...
| メソッド                   | 種別            | 内容                                 |
| -------------------------- | --------------- | ------------------------------------ |
| `startGame(playerColors:)` | BroadcastMethod | ゲーム開始・色の割り当てを全員に通知 |
| `endGame`                  | BroadcastMethod | ゲーム終了を全員に通知               |
...
| メソッド                                  | 種別                            | 内容                                     |
| ----------------------------------------- | ------------------------------- | ---------------------------------------- |
| `spawnCube(id:position:color:spawnTime:)` | BroadcastMethod / UnicastMethod | キューブ出現を全員に同期（座標変換あり） |
| `despawnCube(id:)`                        | BroadcastMethod                 | キューブ消滅を全員に同期                 |
| `touchCube(id:playerId:)`                 | BroadcastMethod                 | タッチ検知を全員に通知                   |

`spawnCube` は `RPCTransformableUnicastMethod` に準拠し、`applying(affineMatrix:)` で `position` にアフィン変換を適用する。

```swift
func applying(affineMatrix: simd_float4x4) -> Self {
    switch self {
    case .spawnCube(let p):
        let transformed = affineMatrix * simd_float4(p.position, 1)
        return .spawnCube(.init(
            id: p.id,
            position: simd_float3(transformed.x, transformed.y, transformed.z),
            color: p.color,
...
| メソッド              | 種別            | 内容                   |
| --------------------- | --------------- | ---------------------- |
| `addScore(playerId:)` | BroadcastMethod | スコア +1 を全員に同期 |
| `resetScore`          | BroadcastMethod | スコアリセット         |
...
```
spawnCube broadcast 受信
  → spawnTime を記録してローカルに RealityKit エンティティを生成（1cm）
  → 毎フレーム elapsed = now - spawnTime を計算
  → size = lerp(0.01, 1.0, elapsed / growDuration)
  → elapsed > growDuration + displayDuration になったら自律的に despawnCube broadcast
```

成長・消滅のアニメーションは各デバイスがローカルで計算するため、通信不要。
...
```
プレイヤーが自分の色のキューブに Spatial Tap
  1. despawnCube(id) を run(syncAll: broadcast)
  2. addScore(myPlayerId) を run(syncAll: broadcast)
  3. 新しい座標をローカルで生成
  4. spawnCube(...) を run(transforming: .all) で各デバイスの座標系に変換して送信
```

### 同時タッチの処理

- `despawnCube` を受信済みの cubeId への2回目の受信は無視（べき等）
- スコアは最初の `touchCube` broadcast を受け入れ、同一 cubeId の2回目以降は無視
...
| 操作                         | 使うメソッド                     | 理由                                 |
| ---------------------------- | -------------------------------- | ------------------------------------ |
| ゲーム開始・終了             | `run(syncAll: BroadcastRequest)` | 全員＋自分に同期                     |
| キューブ消滅・スコア加算     | `run(syncAll: BroadcastRequest)` | 全員＋自分に同期                     |
| キューブ出現（座標変換あり） | `run(transforming: .all, ...)`   | 各デバイスの座標系に変換して unicast |
...
```swift
@Observable class CubeHandler { ... }
@Observable class ScoreHandler { ... }
@Observable class GameHandler { ... }
```
...
### 3. ImmersiveSpace を SharedCoordinateImmersiveView でラップ
...
### 4. affineMatrixProvider は自動設定される

`SharedCoordinateImmersiveView` を使用している場合、`rpcModel.affineMatrixProvider` は自動でセットされるため `run(transforming:)` をそのまま使用できる。
...
<array>
    <string>_CubeGame._tcp</string>
    <string>_CubeGame._udp</string>
</array>
    ...
    var body: some View {
        RealityView { content in
            ...
            if let unapprovedCapabilities, unapprovedCapabilities.anchor.contains(.hand) {
                ...
                let sensorMesh = MeshResource.generateSphere(radius: 0.02)
                let sensorMaterial = SimpleMaterial(color: .clear, isMetallic: false)
                let touchSensor = ModelEntity(mesh: sensorMesh, materials: [sensorMaterial])
                
                let collisionShape = ShapeResource.generateSphere(radius: 0.02)
                touchSensor.components[CollisionComponent.self] = CollisionComponent(
                    shapes: [collisionShape],
                    isStatic: false
                )
                
                // Create anchor entity for right hand index finger tip
                let indexFingerTipAnchor = AnchorEntity(
                    .hand(.right, location: .indexFingerTip),
                    trackingMode: .predicted
                )
                
                // Add touch sensor to finger anchor
                indexFingerTipAnchor.addChild(touchSensor)
                content.add(indexFingerTipAnchor)

                // NOTE: indexFingerTipAnchor を使ってトラッキングできる

                _ = content.subscribe(to: CollisionEvents.Began.self, on: touchSensor) { (event: CollisionEvents.Began) in
                    // コリジョンイベントはここで認識する
                }
            }
        }
...
| `NSBonjourServices`              | `_<serviceType>._tcp`, `_<serviceType>._udp`                  |

> [!IMPORTANT] **`serviceType` はアプリごとに固有の識別子を使用してください。**  
> `NSBonjourServices` に登録する値は `_<serviceType>._tcp` / `_<serviceType>._udp` の形式で、`PeerManager` の `serviceType` 引数と必ず一致させてください。  
> `serviceType` は **1〜15 文字の ASCII 英数字またはハイフン** で構成する必要があります（MultipeerConnectivity の制約）。  
> デフォルト値は `"ImmersiveRPCKit"` ですが、複数のアプリが同じ `serviceType` を使うと意図しない Peer と接続してしまうため、本番アプリでは固有の値に変更してください。
...
struct MyApp: App {
    ...
    init() {
        peerManager = PeerManager(
            ...
            receiveExchangeDataWrapper: receiveWrapper,
            mcPeerIDUUIDWrapper: peerWrapper,
            serviceType: "MyApp"  // Info.plist の NSBonjourServices に登録した値と一致させる
        )
        rpcModel = RPCModel(
            sendExchangeDataWrapper: sendWrapper,
            receiveExchangeDataWrapper: receiveWrapper,
